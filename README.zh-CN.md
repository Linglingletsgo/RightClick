...
1. 从 GitHub release 下载 `RightClick-0.2.1.dmg`。
2. 打开 DMG，将 `RightClick.app` 拖入 `Applications`。
3. 启动 `RightClick.app`。
   - RightClick 目前尚未进行 Apple 公证。如果 macOS 阻止首次打开，请在 `Applications` 中右键点击 `RightClick.app`，选择“打开”，再在确认窗口中选择“打开”。
   - 如果仍被阻止，请打开“系统设置 > 隐私与安全性”，在 RightClick 提示处选择“仍要打开”。
4. 打开“系统设置 > 通用 > 登录项与扩展 > 扩展”，选择 `RightClick`，启用“文件提供程序”，然后点击“完成”。
5. 打开“系统设置 > 隐私与安全性 > 辅助功能”，添加并启用 `/Applications/RightClick.app`。`Show/Hide Hidden Folders` 需要该权限来发送 Finder 原生 `Shift-Command-.` 快捷键。
6. 如果右键菜单没有立即出现，重启 Finder。
...
- 隐藏项目开关会通过后台命令路径模拟 Finder 原生 `Shift-Command-.` 快捷键。macOS 首次发送该快捷键时可能要求给 `RightClick` 授予 Accessibility 权限。
...
DMG 输出位置为 `dist/RightClick-0.2.1.dmg`。

## 说明

- RightClick 面向个人使用，不做插件系统或大众分发平台。
- 应用基于 macOS Finder Sync，因此菜单只会出现在监听目录内。
- 应用目前是面向个人使用的本地签名版本，尚未进行 Apple 公证。如果面向公众分发，需要使用 Developer ID 证书并完成 Apple notarization。
- `0.2.1` 版本目标系统为 macOS 26。
